...
public class CertificateUtils {
    ...
    public static void addCertificatesFromSpecifiedTruststoreToDefaultTruststore(Path customTruststorePath,
            String customTruststorePassword) throws Exception {
    ...
    private static SSLContext createNewSslContext(KeyStore defaultTrustStore)
        ...
        TrustManagerFactory tmf = TrustManagerFactory
                .getInstance(TrustManagerFactory
                        .getDefaultAlgorithm());
        tmf
                .init(defaultTrustStore);
        SSLContext newContext = SSLContext
                .getInstance("TLS");
        newContext
                .init(null, tmf
    ...
    private static void updateDefaultTruststore(KeyStore customTrustStore, KeyStore defaultTrustStore)
        ...
        Enumeration<String> aliases = customTrustStore
                .aliases();
        while (aliases
                .hasMoreElements()) {
            ...
            if (customTrustStore
                ...
                X509Certificate cert = (X509Certificate) customTrustStore
                        .getCertificate(alias);
                if (cert != null) {
                    defaultTrustStore
                            .setCertificateEntry("custom-" + alias, cert);
                }
            }
        }
    }
    ...
    private static KeyStore loadCustomTruststore(Path customTrustStorePath, String customTruststorePassword)
        ...
        KeyStore customTrustStore = KeyStore
                .getInstance("PKCS12");
        try (FileInputStream inputStream = new FileInputStream(customTrustStorePath
                .toFile())) {
            char[] password = null;
            if (customTruststorePassword != null) {
                password = customTruststorePassword
                        .toCharArray();
            }
            customTrustStore
    ...
    private static KeyStore getDefaultTrustStore()
            throws KeyStoreException, IOException, NoSuchAlgorithmException, CertificateException {
        String defaultTruststorePath = System
                .getProperty("java.home") + "/lib/security/cacerts";
        char[] defaultPassword = JAVA_DEFAULT_TRUSTSTORE_PASSWORD
                .toCharArray();

        KeyStore defaultTrustStore = KeyStore
                .getInstance(KeyStore
                        .getDefaultType());
        try (FileInputStream inputStream = new FileInputStream(defaultTruststorePath)) {
            defaultTrustStore
                    .load(inputStream, defaultPassword);
        }
        return defaultTrustStore;
    }

    public static X509CertificateHolder loadPemCertificateFromClasspath(String path) {
        URL resourceUrl = CertificateUtils.class
                .getResource(path);
        if (resourceUrl == null) {
            throw new IllegalArgumentException("Could not find certificate \"%s\" on classpath"
                    .formatted(path));
        }
        try (PEMParser pemParser = new PEMParser(new InputStreamReader(resourceUrl
                .openStream()))) {
    ...
    public static PrivateKey loadECPrivateKeyFromClasspath(String path) {
        ...
        try (InputStream inputStream = resourceUrl
            ...
            pem = new String(inputStream
                    .readAllBytes(), StandardCharsets.UTF_8);
        }

        // Extract "EC PARAMETERS" block from PEM file
        Pattern paramsPattern = Pattern
                .compile("-----BEGIN EC PARAMETERS-----(.*?)-----END EC PARAMETERS-----", Pattern.DOTALL);
        Matcher paramsMatcher = paramsPattern
                .matcher(pem);
        if (!paramsMatcher
                .find()) {
        ...
        String paramsBase64 = paramsMatcher
                .group(1)
                .replaceAll("\\s+", "");
        byte[] paramsBytes = Base64
                .getDecoder()
                .decode(paramsBase64);
        ASN1Primitive paramsAsn1 = ASN1Primitive
                .fromByteArray(paramsBytes);
        ASN1ObjectIdentifier curveOid;
        if (paramsAsn1 instanceof ASN1ObjectIdentifier oid) {
            curveOid = oid;
        } else {
            throw new IllegalArgumentException("Invalid EC PARAMETERS block — expected OID but got: " + paramsAsn1
                    .getClass());
        }

        // Extract "EC PRIVATE KEY" block from PEM file
        Pattern keyPattern = Pattern
                .compile("-----BEGIN EC PRIVATE KEY-----(.*?)-----END EC PRIVATE KEY-----", Pattern.DOTALL);
        Matcher keyMatcher = keyPattern
                .matcher(pem);
        if (!keyMatcher
                .find()) {
        ...
        PrivateKeyInfo privateKeyInfo = new PrivateKeyInfo(
                ...
                ecPrivateKey);
        PKCS8EncodedKeySpec spec = new PKCS8EncodedKeySpec(privateKeyInfo
                .getEncoded());
