...
public class CrlRestClientFactory {
    ...
    public RestClient create(CrlConfigurationProperties.CrlDownload crl) {
        ...
        SslBundle sslBundle =
            crl.tlsTruststoreBundle() != null ? sslBundles.getBundle(crl.tlsTruststoreBundle()) : null;
        ...
        ClientHttpRequestFactory requestFactory = clientHttpRequestFactoryBuilder.build(settings);

        return restClientBuilder.baseUrl(url.toString()).requestFactory(requestFactory).build();
    }
}
