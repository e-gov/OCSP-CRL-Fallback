...
public class OcspMapper {
    ...
    public OcspLogger.OcspResponseContext toOcspContext(X509CertificateHolder issuerCertificate,
            CertificateID certificateId, X509CRLHolder crlHolder, CertificateStatus certificateStatus,
            OCSPResp ocspResp) {
        return OcspLogger.OcspResponseContext
                .builder()
                .issuerCn(X509Utils
                        .getSubjectCN(issuerCertificate))
                .serialNumber(certificateId
                        .getSerialNumber()
                        .toString(16))
                .certStatus(getCertificateStatusName(certificateStatus)
                        .toString())
                .producedAt(getProducedAt(ocspResp))
                .thisUpdate(crlHolder
                        .getThisUpdate())
                .nextUpdate(crlHolder
                        .getNextUpdate())
                .crlNumber(X509Utils
                        .getCrlNumber(crlHolder))
                .build();
    }
    ...
    private static Date getProducedAt(OCSPResp ocspResp) {
        ...
        try {
            basicResponse = (BasicOCSPResp) ocspResp
                    .getResponseObject();
