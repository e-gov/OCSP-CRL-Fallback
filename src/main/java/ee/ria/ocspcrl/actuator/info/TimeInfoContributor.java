package ee.ria.ocspcrl.actuator.info;
...
public class TimeInfoContributor implements InfoContributor {
    ...
    public void contribute(Info.Builder builder) {
        builder
                ...
                .withDetail("currentTime", OffsetDateTime
                        .now(ZoneOffset.UTC));
    }

    private OffsetDateTime getServiceStartTime() {
        TimeGauge startTime = meterRegistry
                .find("process.start.time")
                .timeGauge();
        if (startTime == null) {
            throw new IllegalStateException("Failed to get application start time");
        }
        long startTimeEpochMilli = Double
                .valueOf(startTime
                        .value(TimeUnit.MILLISECONDS))
                .longValue();
        Instant startTimeInstant = Instant
                .ofEpochMilli(startTimeEpochMilli);
        return OffsetDateTime
                .ofInstant(startTimeInstant, ZoneOffset.UTC);
    }
}
