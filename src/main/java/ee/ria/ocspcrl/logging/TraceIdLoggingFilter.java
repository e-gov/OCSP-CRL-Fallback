...
public class TraceIdLoggingFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        String requestTraceId = ElasticApm
                .currentTransaction()
                .getTraceId();
        boolean elasticApmTraceIdExists = StringUtils
                .isNotEmpty(requestTraceId);
        if (!elasticApmTraceIdExists) {
            // Use same format as Elastic APM Agent.
            requestTraceId = RandomStringUtils
                    .random(32, "0123456789abcdef");
        }
        ...
        request
                .setAttribute(REQUEST_ATTRIBUTE_NAME_REQUEST_ID, requestTraceId);

        if (!elasticApmTraceIdExists) {
            MDC
                    .put(MDC_ATTRIBUTE_KEY_REQUEST_TRACE_ID, requestTraceId);
        }
        try {
            filterChain
            ...
            if (!elasticApmTraceIdExists) {
                MDC
                        .remove(MDC_ATTRIBUTE_KEY_REQUEST_TRACE_ID);
            }
        }
    }
}
