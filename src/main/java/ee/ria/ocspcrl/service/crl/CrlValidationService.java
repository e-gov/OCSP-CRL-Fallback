...
public class CrlValidationService {
    ...
    public boolean shouldUse(String chainName, X509CRLHolder crlHolder) {
        try {
            log
                    .debug("Validating CRL: {}", chainName);
            validateSignature(crlHolder, chainName);
        } catch (CrlValidationException e) {
            log
                    .warn("Failed to validate CRL for {}: {}", chainName, e
                            .getMessage());
            return false;
        } catch (Exception e) {
            log
    ...
    private void validateSignature(X509CRLHolder crlHolder, String chainName) {
        CertificateChain certificateChain = properties
                .certificateChain(chainName);
        X509CertificateHolder certificate = certificateChain
                .issuerCertificate();
        ...
        try {
            ...
            if (!crlHolder
                    .isSignatureValid(verifierProvider)) {
    ...
    private boolean validateNextUpdate(X509CRLHolder crlHolder, String chainName) {
        X509CRLHolder previousValidatedCrl = crlCache
                .getCrl(chainName);
        if (previousValidatedCrl == null)
            return true;

        Date newNextUpdate = crlHolder
                .getNextUpdate();
        Date previousNextUpdate = previousValidatedCrl
                .getNextUpdate();
        ...
        if (newNextUpdate
                .before(previousNextUpdate)) {
            log
                    .warn("New Next Update is before the previous one");
            return false;
        }
        return !newNextUpdate
                .equals(previousNextUpdate);
    }
}
