...
public class CrlLoadingService {
    ...
    public void loadCrlsFromDisk() {
        log
                .info("Loading CRLs from disk...");
        for (var chain : properties
                .certificateChains()) {
            try {
                CrlCache.CrlInfo crlInfo = loadCrlInfoFromDisk(chain
                        .name());
                if (crlInfo == null) {
                    continue;
                }
                ...
                crlCache
                        .updateCrlAndHeaders(chain
                                ...
                                crlInfo
                                        .crlHeaders());
                log
                        .info("Loaded CRL for {}", chain
                                .name());
            } catch (Exception e) {
                log
                        .atError()
                        .setCause(e)
                        .log("Failed to load CRL for {}", chain
                                .name());
            }
        }
        log
                .info("Loading CRLs from disk completed");
    ...
    private CrlCache.CrlInfo loadCrlInfoFromDisk(String chainName) throws IOException {
        ...
        try {
            crlHolder = fileService
                    .deserializeCrlFromFile(chainName, FileService.FileType.VALIDATED);
            crlHeaders = fileService
                    .deserializeCrlHeadersFromFile(chainName, FileService.FileType.VALIDATED);
        } catch (NoSuchFileException e) {
            log
                    .info("Cannot find file {}", e
                            .getMessage());
            return null;
        }

        if (!crlValidationService
                .shouldUse(chainName, crlHolder)) {
            log
                    .debug("Aborted loading CRL from disk for chain {}", chainName);
            return null;
        }
