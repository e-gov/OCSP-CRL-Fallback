    ...
    public void init() throws GeneralSecurityException {
        ...
        if (this.ocspSigningKey == null) {
            throw new IllegalStateException(
                "Failed to find PrivateKey in SslBundle '" + OCSP_BUNDLE_NAME + "' with alias " + ocspKeystoreEntryAlias
            );
        }
        if (this.ocspSigningCert == null) {
            throw new IllegalStateException(
                "Failed to find X509Certificate in SslBundle '" +
                    OCSP_BUNDLE_NAME +
                    "' with alias " +
                    ocspKeystoreEntryAlias
            );
        }
    }
}
