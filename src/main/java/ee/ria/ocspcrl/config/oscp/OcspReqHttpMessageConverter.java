...
public class OcspReqHttpMessageConverter extends AbstractHttpMessageConverter<OCSPReq> {
    ...
    static final DataSize MAX_BODY_SIZE = DataSize
            .ofKilobytes(10);

    public OcspReqHttpMessageConverter() {
        super(MediaType
                .valueOf(OCSP_REQUEST_CONTENT_TYPE));
    }

    @Override
    protected boolean supports(@NonNull Class<?> clazz) {
        return OCSPReq.class
    ...
    protected OCSPReq readInternal(@NonNull Class<? extends OCSPReq> clazz, @NonNull HttpInputMessage inputMessage)
        ...
        try (InputStream requestBodyStream = inputMessage
                .getBody()) {
            byte[] bytes = requestBodyStream
                    .readNBytes(Math
                            .toIntExact(MAX_BODY_SIZE
                                    .toBytes()));
            if (requestBodyStream
                    .read() != -1) {
                throw new HttpMessageNotReadableException("Expected OCSP request to be no larger than " + MAX_BODY_SIZE,
                        inputMessage);
            }
